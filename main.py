...
def main():
    ...
    claw_position = (
        analog_read(grab_dial_pin) / 10.23
    )  # Convert grabber dial value to a percent value.
    ...
    while True:  # Open loop so claw positions and user info can be continually changed.
        ...
        position_list.append(
            claw_position
        )  # Add percent claw position to list of samples.
        sum_list += claw_position  # Add percent claw position to the sum variable
        if (
            check_button_one(button_pin) == True
        ):  # Loop will end when button is pressed, ending the program.
            break
